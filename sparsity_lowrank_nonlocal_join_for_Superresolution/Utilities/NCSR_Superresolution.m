...
hr_im    =   Superresolution(lrim, par, ori_im, hr_im, 0, 40);
...
for k    =  1:K    % K
    ...
    for  iter    =   1 : 15   % par.iters
        ...
        [rim, wei, U_arr]      =   Low_rank_appro(reshape(f,h,w), par, blk_arr', U_arr, iter, flag1 );   
        f     =    (rim+beta*reshape(f,h,w))./(wei+beta);
